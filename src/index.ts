  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      if (path === '/health' && request.method === 'GET') {
        ...
        // Todoist health
        try {
          const inbox = await todoist.getInboxProject();
          if (inbox) {
            const tasks = await todoist.getInboxTasks(true, env.SYNC_METADATA);
            result.todoist.ok = true;
            result.todoist.inboxCount = (tasks || []).length;
          }
        } catch (err) {
          result.ok = false;
          result.todoist.error = err instanceof Error ? err.message : 'Todoist error';
        }

        // KV health (D1KV adapter)
        try {
          const key = `health:ping:${Date.now()}`;
          await env.SYNC_METADATA.put(key, 'ok', { expirationTtl: 60 });
          const val = await env.SYNC_METADATA.get(key);
          result.kv.ok = val === 'ok';
          if (val !== 'ok') result.ok = false;
        } catch (err) {
          result.ok = false;
          result.kv.error = err instanceof Error ? err.message : 'KV error';
        }

        // D1 health
        try {
          const row = await env.DB.prepare('SELECT 1 as v').first<any>();
          result.d1.ok = row?.v === 1;
          if (!result.d1.ok) result.ok = false;
        } catch (err) {
          result.ok = false;
          result.d1.error = err instanceof Error ? err.message : 'D1 error';
        }
      ...
      // Metrics endpoint: GET /metrics?hours=24
      if (path === '/metrics' && request.method === 'GET') {
        const hoursParam = url.searchParams.get('hours');
        const hours = Math.max(1, Math.min(168, parseInt(hoursParam || '24', 10) || 24));
        try {
          ...
          return new Response(JSON.stringify({ error: 'Failed to load metrics', message: error instanceof Error ? error.message : 'Unknown error' }), {
            status: 500,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' }
          });
        }
      ...
      if (path === '/obsidian/sync' && request.method === 'POST') {
        try {
          const { tasks, vault, filter, syncMode = 'incremental' } = await request.json() as any;
          
          // Import Obsidian utilities
          const { obsidianToTodoist, obsidianToThings, createObsidianTaskFingerprint } = await import('./obsidian');
          
          // Initialize batch sync manager and Todoist client
          const batchSync = new BatchSyncManager(env);
          await batchSync.loadState();
          // Use unified Todoist client that reads credentials from env
          const todoist = new TodoistClient(env);
          
          const results = await Promise.all(tasks.map(async (obsidianTask: any) => {
            try {
              // Generate fingerprint for the task
              const fingerprint = createObsidianTaskFingerprint(obsidianTask);
              
              // Check if task already exists via fingerprint
              const existingMapping = await batchSync.getMapping(fingerprint);
              
              if (existingMapping) {
                // Task already synced, skip it
                return { 
                  obsidianId: obsidianTask.id, 
                  status: 'already_synced',
                  todoistId: existingMapping.todoistId,
                  thingsId: existingMapping.thingsId
                };
              }
              
              // Create in Todoist with proper label
              const todoistData = obsidianToTodoist(obsidianTask);
              const todoistTask = await todoist.createTask({
                ...todoistData,
                labels: [...(todoistData.labels || []), 'synced-from-obsidian']
              } as any);
              
              // Store mapping with fingerprint
              await batchSync.addMapping({
                todoistId: todoistTask.id,
                thingsId: '', // Will be filled when Things sync runs
                fingerprint: {
                  primaryHash: fingerprint,
                  titleVariations: [],
                  fuzzySearchable: obsidianTask.content.toLowerCase()
                },
                lastSynced: new Date().toISOString(),
                source: 'exact',
                obsidianId: obsidianTask.id
              } as TaskMapping & { obsidianId?: string });
              ...
              console.error('Error syncing task:', obsidianTask.id, error);
              return {
                obsidianId: obsidianTask.id,
                status: 'error',
          ...
          return new Response(JSON.stringify({ 
            error: 'Failed to sync Obsidian tasks',
            message: error instanceof Error ? error.message : 'Unknown error'
          }), {
            status: 500,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      ...
      if (path === '/obsidian/tasks' && request.method === 'GET') {
        try {
          // Initialize Todoist client
          // Use unified Todoist client that reads credentials from env
          const todoist = new TodoistClient(env);
          
          // First, get the inbox project ID
          const projectsResponse = await fetch(`${env.TODOIST_API_URL}/projects`, {
            headers: {
          ...
          // Get all active tasks from the inbox
          const response = await fetch(`${env.TODOIST_API_URL}/tasks?project_id=${inboxProject.id}`, {
            headers: {
              'Authorization': `Bearer ${env.TODOIST_API_TOKEN}`,
            },
          });
          ...
          const { todoistToObsidian } = await import('./obsidian');
          
          // Filter out tasks that originated from Obsidian
          for (const task of tasks) {
            ...
            // Skip tasks that have the synced-from-obsidian label
            if (task.labels && task.labels.includes('synced-from-obsidian')) {
              continue;
            }
            
            // Convert to Obsidian format
            const obsidianTask = todoistToObsidian(task);
            obsidianTasks.push(obsidianTask);
          }
          
          return new Response(JSON.stringify({
            tasks: obsidianTasks,
            count: obsidianTasks.length
          }), {
            headers: { ...corsHeaders, 'Content-Type': 'application/json' }
          });
        } catch (error) {
          return new Response(JSON.stringify({
            error: 'Failed to fetch tasks for Obsidian',
            message: error instanceof Error ? error.message : 'Unknown error'
          }), {
            status: 500,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      ...
      if (path === '/obsidian/mark-synced' && request.method === 'POST') {
        try {
          const { obsidianIds } = await request.json() as { obsidianIds: string[] };
          ...
          // Mark tasks as synced by adding labels in Todoist
          let marked = 0;
          for (const obsidianId of obsidianIds) {
            try {
              // Find the mapping for this Obsidian task
              const allMappings = await batchSync.getAllMappings();
              const mapping = allMappings.find((m: any) => m.obsidianId === obsidianId);
              
              if (mapping && mapping.todoistId) {
                // Add the synced-to-obsidian label to prevent re-sync
                const task = await todoist.getTask(mapping.todoistId);
                if (task && !task.labels.includes('synced-to-obsidian')) {
                  await todoist.updateTask(mapping.todoistId, {
                    labels: [...(task.labels || []), 'synced-to-obsidian']
                  });
                  marked++;
                }
              }
            } catch (error) {
              console.error(`Failed to mark task ${obsidianId} as synced:`, error);
            }
          }
          ...
          return new Response(JSON.stringify({
            marked,
            total: obsidianIds.length
          }), {
            headers: { ...corsHeaders, 'Content-Type': 'application/json' }
          });
        } catch (error) {
          return new Response(JSON.stringify({
            error: 'Failed to mark Obsidian tasks as synced',
            message: error instanceof Error ? error.message : 'Unknown error'
          }), {
            status: 500,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      ...
      // Obsidian: Mark tasks as completed in Todoist and Things
      if (path === '/obsidian/complete' && request.method === 'POST') {
        try {
          ...
          for (const obsidianId of taskIds) {
            try {
              // Find the mapping for this Obsidian task
              const allMappings = await batchSync.getAllMappings();
              const mapping = allMappings.find((m: any) => m.obsidianId === obsidianId);
              ...
              if (mapping) {
                // Complete in Todoist if mapped
                if (mapping.todoistId) {
                  try {
                    await todoist.completeTask(mapping.todoistId);
                    completed++;
                    results.push({ obsidianId, todoistId: mapping.todoistId, status: 'completed' });
                  } catch (error) {
                    console.error(`Failed to complete Todoist task ${mapping.todoistId}:`, error);
                    errors++;
                    results.push({ obsidianId, todoistId: mapping.todoistId, status: 'error', error: error.message });
                  }
                }
                
                // TODO: Also complete in Things when AppleScript support is added
                if (mapping.thingsId) {
                  // Would need to trigger AppleScript via a webhook or scheduled task
                  results.push({ obsidianId, thingsId: mapping.thingsId, status: 'pending_things_sync' });
                }
              } else {
                notFound++;
                results.push({ obsidianId, status: 'not_found' });
              }
            } catch (error) {
              console.error(`Failed to process completion for ${obsidianId}:`, error);
              errors++;
              results.push({ obsidianId, status: 'error', error: error.message });
            }
          }
          
          return new Response(JSON.stringify({
            summary: { completed, notFound, errors },
            results
          }), {
            headers: { ...corsHeaders, 'Content-Type': 'application/json' }
          });
          ...
          return new Response(JSON.stringify({
            error: 'Failed to complete Obsidian tasks',
            message: error instanceof Error ? error.message : 'Unknown error'
          }), {
            status: 500,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      ...
      // Obsidian: Delete tasks from Todoist and Things
      if (path === '/obsidian/delete' && request.method === 'POST') {
        try {
          const { taskIds } = await request.json() as { taskIds: string[] };
          ...
          for (const obsidianId of taskIds) {
            try {
              ...
              if (mapping) {
                // Delete from Todoist if mapped
                if (mapping.todoistId) {
                  try {
                    await todoist.deleteTask(mapping.todoistId);
                    deleted++;
                    results.push({ obsidianId, todoistId: mapping.todoistId, status: 'deleted' });
                  } catch (error) {
                    // Task might already be deleted
                    if (error.message.includes('404')) {
                      deleted++;
                      results.push({ obsidianId, todoistId: mapping.todoistId, status: 'already_deleted' });
                    } else {
                      console.error(`Failed to delete Todoist task ${mapping.todoistId}:`, error);
                      errors++;
                      results.push({ obsidianId, todoistId: mapping.todoistId, status: 'error', error: error.message });
                    }
                  }
                ...
                // Remove the mapping
                await batchSync.removeMapping(mapping.fingerprint?.primaryHash || obsidianId);
                
                // TODO: Also delete from Things when AppleScript support is added
                if (mapping.thingsId) {
                  results.push({ obsidianId, thingsId: mapping.thingsId, status: 'pending_things_sync' });
                }
              } else {
              ...
              console.error(`Failed to process deletion for ${obsidianId}:`, error);
              errors++;
              results.push({ obsidianId, status: 'error', error: error.message });
            }
          }
          ...
          return new Response(JSON.stringify({
            error: 'Failed to delete Obsidian tasks',
            message: error instanceof Error ? error.message : 'Unknown error'
          }), {
            status: 500,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      ...
      // Obsidian: Batch sync changes (completions and deletions)
      if (path === '/obsidian/sync-changes' && request.method === 'POST') {
        try {
          const { completed = [], deleted = [], modified = [] } = await request.json() as {
            completed?: string[];
            deleted?: string[];
            modified?: Array<{ id: string; task: any }>;
          };
          ...
          const results = {
            completed: { success: 0, failed: 0, results: [] as any[] },
            deleted: { success: 0, failed: 0, results: [] as any[] },
            modified: { success: 0, failed: 0, results: [] as any[] }
          };
          
          // Process completions
          for (const obsidianId of completed) {
            try {
              const allMappings = await batchSync.getAllMappings();
              const mapping = allMappings.find((m: any) => m.obsidianId === obsidianId);
              ...
              if (mapping?.todoistId) {
                ...
                results.completed.results.push({ obsidianId, todoistId: mapping.todoistId, status: 'completed' });
              } else {
                results.completed.failed++;
                results.completed.results.push({ obsidianId, status: 'not_found' });
              }
            } catch (error) {
              results.completed.failed++;
              results.completed.results.push({ obsidianId, status: 'error', error: error.message });
            }
          }
          
          // Process deletions
          for (const obsidianId of deleted) {
            try {
              const allMappings = await batchSync.getAllMappings();
              const mapping = allMappings.find((m: any) => m.obsidianId === obsidianId);
              ...
              if (mapping) {
                if (mapping.todoistId) {
                  try {
                    ...
                    // Ignore 404 errors (task already deleted)
                    if (!error.message.includes('404')) throw error;
                  }
                }
                await batchSync.removeMapping(mapping.fingerprint?.primaryHash || obsidianId);
                results.deleted.success++;
                results.deleted.results.push({ obsidianId, status: 'deleted' });
              } else {
                results.deleted.failed++;
                results.deleted.results.push({ obsidianId, status: 'not_found' });
              }
            } catch (error) {
              results.deleted.failed++;
              results.deleted.results.push({ obsidianId, status: 'error', error: error.message });
            }
          }
          
          // Process modifications
          for (const { id: obsidianId, task } of modified) {
            try {
              const allMappings = await batchSync.getAllMappings();
              const mapping = allMappings.find((m: any) => m.obsidianId === obsidianId);
              
              if (mapping?.todoistId) {
                const { obsidianToTodoist } = await import('./obsidian');
                const todoistData = obsidianToTodoist(task);
                await todoist.updateTask(mapping.todoistId, todoistData);
                results.modified.success++;
                results.modified.results.push({ obsidianId, todoistId: mapping.todoistId, status: 'updated' });
              } else {
                results.modified.failed++;
                results.modified.results.push({ obsidianId, status: 'not_found' });
              }
            } catch (error) {
              results.modified.failed++;
              results.modified.results.push({ obsidianId, status: 'error', error: error.message });
            }
          }
          ...
          return new Response(JSON.stringify({
            summary: {
              completed: results.completed.success,
              deleted: results.deleted.success,
              modified: results.modified.success,
              errors: results.completed.failed + results.deleted.failed + results.modified.failed
            },
            results
          }), {
            headers: { ...corsHeaders, 'Content-Type': 'application/json' }
          ...
          return new Response(JSON.stringify({
            error: 'Failed to sync Obsidian changes',
            message: error instanceof Error ? error.message : 'Unknown error'
          }), {
            status: 500,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      ...
      if (path === '/sync/status' && request.method === 'GET') {
        ...
        if (stateData) {
          ...
          hashCount = mappingCount;
          thingsMappings = hashCount;
          todoistMappings = hashCount;
          lastUpdated = state.lastUpdated;
          migration = {
            migratedLegacyMappings: state.stats?.migratedLegacyMappings || hashCount,
        ...
        return new Response(JSON.stringify({
          ...
          fingerprint: {
            ...
            migratedLegacyMappings: migration.migratedLegacyMappings,
            pendingLegacyMigration: migration.pendingLegacyMigration
          },
          migration: {
            progress: migration.pendingLegacyMigration === 0 ? '100%' : `${Math.round((migration.migratedLegacyMappings / (migration.migratedLegacyMappings + migration.pendingLegacyMigration)) * 100)}%`,
            isComplete: migration.pendingLegacyMigration === 0 && (taggedTasks.length - taggedTasksWithFingerprints) === 0,
            recommendations: [
              ...(migration.pendingLegacyMigration > 0 ? ['Run POST /migrate to migrate legacy mappings'] : []),
              ...((taggedTasks.length - taggedTasksWithFingerprints) > 0 ? ['Run POST /migrate to migrate tagged tasks'] : []),
              ...(migration.pendingLegacyMigration === 0 && (taggedTasks.length - taggedTasksWithFingerprints) === 0 ? ['Migration complete! System ready for tag-free operation'] : [])
            ]
          },
          timestamp: new Date().toISOString()
        }), {
