...
export class TodoistClient {
  private projectsCache: TodoistProject[] | null = null;
  private inboxProjectCache: TodoistProject | null = null;
  ...
  async getProjects(): Promise<TodoistProject[]> {
    ...
    const projects = await this.request<TodoistProject[]>('/projects');
    this.projectsCache = projects;
    return projects;
  }

  async getInboxProject(): Promise<TodoistProject | undefined> {
    if (this.inboxProjectCache) return this.inboxProjectCache;
    const projects = await this.getProjects();
    const inbox = projects.find(p => p.is_inbox_project);
    if (inbox) this.inboxProjectCache = inbox;
    return inbox;
  }
  ...
  async getTask(taskId: string): Promise<TodoistTask> {
    return await this.request<TodoistTask>(`/tasks/${taskId}`);
  }
  ...
  // Alias for closeTask to match the naming convention
